...
WORKDIR ${BUILD_PATH}
COPY . .
RUN  mkdir -p ${RUN_PATH} && cp -rf ${BUILD_PATH}/_output/* ${RUN_PATH}/

# disable ES and kubelet fetching pods in huatuo-bamai.conf
RUN sed -i -e 's|# Address = "http://127.0.0.1:9200"|Address = ""|' \
-e '/^\[Pod\]/,/^\[/ s|^\([[:space:]]*KubeletClientCertPath = \)|    KubeletReadOnlyPort = 0\n    KubeletAuthorizedPort = 0\n\1|' \
${RUN_PATH}/conf/huatuo-bamai.conf
